...
def generate_lifetime_decay_photon_simulation(
    num_photons,
    lifetime,
    time_range=(0, 12.5),
    time_bins=256,
...
def fit_lifetime_decay(times, counts, initial_guess=None):
    ...
    try:
        params, covariance = curve_fit(
            exponential_model, times, counts, p0=initial_guess
        )
...
def plot_lifetime_decay(
    ...
    log_scale=True,
    residuals=True,
    title="Fluorescence Lifetime Decay",
):
    """
...
def simulate_multi_exponential_decay(
    num_photons, lifetimes, amplitudes, time_range=(0, 10), time_bins=1000, background=0
):
    """
        ...
        for b in bincalc:
            ...
            if __name__ == "__main__":
                ...
                if covariance is not None:
                    ...
                    datastore.append(
                        {
                            "true lifetime": true_lifetime,
                            "fitted lifetime": fitted_lifetime,
                            "time_bins": time_bins,
